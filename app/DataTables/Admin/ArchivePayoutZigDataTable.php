...
class ArchivePayoutZigDataTable extends DataTable
{
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('detail', function ($query) {
                ...
                elseif ($query->transaction_type === 'easypaisa' && $query->status === 'success') {
                    $buttons .= '
                        <a href="' . route('admin.payout.detail', $query->id) . '" class="btn btn-primary btn-sm">Detail</a>
    ...
    public function query()
    {
        ...
        $end   = request()->end_date ?? \Carbon\Carbon::now()->toDateString();

        $model = ArcheivePayout::where('user_id', 4)
